...
target_density = ggplot(df, aes(value, dist, fill = dist)) + 
  ...
  labs(x = expression(paste("Vector correlation between peak position and ",g[max])),
       y = "Distributions") + 
...
target_density
save_plot("plots/target_histogram_ridges.png", target_density, base_height = 7, base_asp = 1.2)
